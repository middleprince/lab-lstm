...
class Config:
    # 数据参数
    feature_columns = list(range(2, 4))     # 要作为feature的列，按原数据从0开始计算
    label_columns = [2, 3]                  # 要预测的列，也即是label data按原数据从0开始计算 
    label_in_feature_index = (lambda x,y: [x.index(i) for i in y])(feature_columns, label_columns)  # 因为feature含有label的index

    predict_length = 10             # 预测序列的长度
    ...
    time_step = 14              # LSTM的time step 序列长度
    ...
    do_continue_train = True    # 每次训练把上一次的final_state作为下一次的init_state,使用道间相似性 
    ...
    train_data_path = "./data/large.csv"
...
class Data:
    ...
    def get_train_and_valid_data(self):
        ...
        label_data = self.norm_data[self.config.predict_length : self.config.predict_length + self.train_num,
...
if __name__=="__main__":
    ...
    parser.add_argument("-p", "--do_predict", default=False, type=bool, help="whether to train")
